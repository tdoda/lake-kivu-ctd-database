...
def compute_avgtrend_array(tnum,dval,data_array,indtime,dz,dvar=0.001,nmin=10,mindur=1):
# tnum: time values (timestamp format)
# dval: depth values [m]
# data_array: data for whih the trends must be computed
# indtime: time indices where the trend should be computed
# dz: new depth step to compute trend
# dvar: step for trends of isolines 
# nmin: minimum number of values needed to compute trend
# mindur [yr]: minimum duration spanned by the data to calculate trend 


    depth_trend=np.arange(dval[0],dval[-1],dz)
     
        
    nlayers=round((dval[-1]-dval[0])/dz)
    nval=round(dz/(dval[1]-dval[0]))
    prof_avg=np.mean(data_array.reshape((nval,-1),order='F'),axis=0).reshape((nlayers,-1),order='F') # Profile with new z resolution(depth_trend), averages in each layer

    trend_avg=(prof_avg[:,indtime[-1]]-prof_avg[:,indtime[0]])/(tnum[indtime[-1]]-tnum[indtime[0]])*3600*24*365
    trend_prof=np.full((len(depth_trend),),np.nan) # Profile of trends
    R2_prof=np.full((len(depth_trend),),np.nan) # Profile of trends
    pfit_prof=np.full((len(depth_trend),2),np.nan) # Profile of trends
    for kz in range(len(depth_trend)): 
        if sum(~np.isnan(prof_avg[kz,indtime]))>nmin: # At least nmin samples
            indval0=np.where(~np.isnan(prof_avg[kz,indtime]))[0][0]# First profile used
            indvalf=np.where(~np.isnan(prof_avg[kz,indtime]))[0][-1]# Last profile used
            if (tnum[indtime[indvalf]]-tnum[indtime[indval0]])>=mindur*365*24*3600: # At least duration of mindur years
                pfit, _, R2=regression_oneline(tnum[indtime]/(3600*24*365),prof_avg[kz,indtime])
                trend_prof[kz]=pfit[0]
                R2_prof[kz]=R2
                pfit_prof[kz,:]=[pfit[0],pfit[1]]
    trend_fit=trend_prof
    return depth_trend, prof_avg, trend_avg, trend_fit, pfit_prof, R2_prof
