...
class ctd_periods:
    def __init__(self):
        ...
        self.variables = {
            ...
            "trenderr_Temp":{'var_name': "trenderr_Temp", 'dim': ('depth_trend','time0_periods'), 'unit': 'degC/yr', 'longname': "Standard error of temperature trend from linear fit"},
            ...
            "trenderr_Cond":{'var_name': "trenderr_Cond", 'dim': ('depth_trend','time0_periods'), 'unit': 'mS.cm-1.yr-1', 'longname': "Standard error of conductivity trend from linear fit"},
            ...
            "trenderr_SALIN":{'var_name': "trenderr_SALIN", 'dim': ('depth_trend','time0_periods'), 'unit': 'g.kg-1.yr-1', 'longname': "Standard error of salinity trend from linear fit"},
            ...
            "trenderr_rho":{'var_name': "trenderr_rho", 'dim': ('depth_trend','time0_periods'), 'unit': 'kg.m-3.yr-1', 'longname': "Standard error of density trend from linear fit"},
            ...
            "trenderr_N2":{'var_name': "trenderr_N2", 'dim': ('depth_trend','time0_periods'), 'unit': 's-2.yr-1', 'longname': "Standard error of N2 trend from linear fit"},
    ...
    def compute_avgtrend(self,database,t0_periods,tf_periods,dz,dvar=[0.001,0.001,0.001,0.001],varnames=["Temp","Cond","SALIN","rho"],nmin=10,mindur=1):
        ...
        for var in varnames:
            ...
            trend_err[var]=np.full((len(depth_trend),len(t0_periods)),np.nan)
            ...
            for kp in range(len(t0_periods)):       
                ...
                if len(indprof)>1:
                    ...
                    err_prof=np.full((len(depth_trend),),np.nan) # Profile of errors
                    ...
                    for kz in range(len(depth_trend)): 
                        if sum(~np.isnan(prof_avg[kz,indprof]))>nmin: # At least nmin samples
                            ...
                            if (database['time'][indprof[indvalf]]-database['time'][indprof[indval0]])>=mindur*365*24*3600: # At least duration of mindur years
                                pfit,_,R2,perr=regression_oneline(database['time'][indprof]/(3600*24*365),prof_avg[kz,indprof])
                                trend_prof[kz]=pfit[0]
                                R2_prof[kz]=R2
                                intercept_prof[kz]=pfit[1]
                                err_prof[kz]=perr[0] # Standard error of the slope
                    trend_fit[var][:,kp]=trend_prof
                    trend_err[var][:,kp]=err_prof
            ...
            self.data["trenderr_"+var]=trend_err[var]
