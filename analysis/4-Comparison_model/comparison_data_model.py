...
for kvar in [0,1,2,3]:  
    ...
    for kz in range(len(depthval)): 
        if np.sum(~np.isnan(data_model[kvar][kz,:]))>2:
            pfit,_,_=regression_oneline(tval/(3600*24*365),data_model[kvar][kz,:])
            trend_fit_model[kvar][kz]=pfit[0]

#%% Compute isopycnals displacements from model (for temp, salin and rho)
...
for kvar in range(len(varnames)):
    ...
    trend_iso_avg,trend_iso_fit,z_iso,var_trend=compute_iso_displacements(tval,depthval,data_model[kvar],dvar[kvar],delta_smooth=1,zmin=0,nmin=2,mindur=1)
